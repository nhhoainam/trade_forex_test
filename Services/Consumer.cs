...
public class Consumer
{
    ...
    {
        var queueName = "sendMessage"; // Queue name
    ...
    public void HotFix()
    {
        ...
        var nameQueueDLX = "sendMessage_DLX"; // Queue name DLX
        var queueExchangeDLX = "sendMessage_exchange_DLX"; // Exchange name DLX
        var notiQueueHotFix = "sendMessageNotificationQueueHotFix";
        ...
        consumer.Received += async (model, ea) =>
        {
            ...
            var message = Encoding.UTF8.GetString(body);
            // Xử lý dữ liệu trong đây
            Console.WriteLine($"HotFix: {message}");
            var json = JsonConvert.DeserializeObject<Message>(message);
            await _producer.SendMessage(json);
            // Xử lý dữ liệu trong đây
